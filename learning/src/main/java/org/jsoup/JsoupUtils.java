...
public class JsoupUtils {
     ...
     * @return url in javascript.
     */
    public static String parsePostUrlFromJS(HttpResponse response, String baseUri) {
        HttpEntity responseEntity = response.getEntity();
        Assert.notNull(responseEntity);
        ...
        try {
            ...
            url = parsePostUrlFromJS(inStream, charset, baseUri);
        } catch (IOException e) {
            logger.error(null, e);
        } finally {
            IOUtils.closeQuietly(inStream);
     ...
     * @throws IOException IO exception.
     */
    public static String parsePostUrlFromJS(InputStream inStream, String charsetName, String baseUri)
            throws IOException {
        String url = null;
        Document doc = Jsoup.parse(inStream, charsetName, baseUri);
        ...
        Pattern p = Pattern.compile("(?is)var\\surl\\s=\\s\"(.+?)\""); // Regex for the value of the key
        Matcher m = p.matcher(script.html()); // you have to use html here and NOT text! Text will drop the 'key' part
        while (m.find()) {
            logger.trace(m.group()); // the whole key ('key = value')
            url = m.group(1); // value only
